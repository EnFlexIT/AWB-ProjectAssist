  ...
  stages {
    stage('Snapshot Build & Deploy for Java 21') {
      steps {
        ...
        sh 'mvn --version'
        sh 'mvn clean install -P p2DeployClean -f de.enflexit.tools.core -Dtycho.localArtifacts=ignore -Dtycho.localArtifacts=ignore -Dtycho.p2.transport.min-cache-minutes=0'
