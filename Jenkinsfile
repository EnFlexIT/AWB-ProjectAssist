  ...
  stages {
    stage('Snapshot Build & Deploy for Java 21') {
      steps {
        ...
        sh 'mvn --version'
        sh 'mvn clean install -P p2Deploy -U -f de.enflexit.tools -Dtycho.localArtifacts=ignore -Dtycho.localArtifacts=ignore -Dtycho.p2.transport.min-cache-minutes=0'
